...
import fullMealsImg from '../assets/full_meals.jpg';
import varietyRiceComboImg from '../assets/variety_rice_combo.jpg';
import parottaImg from '../assets/parotta.jpg';
import specialFoodImg from '../assets/special_food.jpg';
...
const Dashboard = () => {
  const [menuItems, setMenuItems] = useState([
    { name: 'Full Meals', price: '₹10', picture: fullMealsImg },
    { name: 'Variety Rice Combo', price: '₹80', picture: varietyRiceComboImg },
    { name: 'Parotta', price: '₹60', picture: parottaImg },
    { name: 'Special Food', price: '₹120', picture: specialFoodImg },
    { name: 'Biryani', price: '₹150', picture: biryaniImg },
  ...
  const [newItem, setNewItem] = useState({ name: '', price: '', picture: '' });
  ...
  const closeModal = () => {
    ...
    setNewItem({ name: '', price: '', picture: '' });
    setSelectedMenuItem('');
  };

  const handleAddMenuItem = () => {
    if (newItem.name && newItem.price && newItem.picture) {
  ...
  const handleDeleteMenuItem = () => {
    const updatedMenuItems = menuItems.filter(item => item.name !== selectedMenuItem);
    setMenuItems(updatedMenuItems);

    setSelectedItems(prevSelectedItems => {
      const updatedSelectedItems = { ...prevSelectedItems };
      Object.keys(updatedSelectedItems).forEach(day => {
        if (updatedSelectedItems[day].name === selectedMenuItem) {
          updatedSelectedItems[day] = updatedMenuItems[0] || null; // Replace with the first available item or null
        }
      });
  ...
  const handleUpdateDayMenu = () => {
    const selectedItem = menuItems.find(item => item.name === selectedMenuItem);
              ...
              <div key={day} className="menu-item" id={`menu-item-${day}`}>
                ...
                {selectedItems[day] ? (
                  <>
                    ...
                    <p id={`product-name-${day}`}>{selectedItems[day].name}</p>
